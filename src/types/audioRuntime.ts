...
export type MidiHandleResult = {
  ...
  consumed?: boolean;
  /** New events to emit from this node (will be routed to outgoing connections). */
  emit?: MidiEvent[];
};
...
export type AudioNodeInstance<TNode extends GraphNode = GraphNode> = {
   ...
   */
  handleMidi?: (event: MidiEvent, portId: string | null, state: TNode["state"]) => MidiHandleResult | void;
